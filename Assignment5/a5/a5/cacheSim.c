...
u_int32_t read(u_int32_t address)
{
	...
	unsigned int L2setID = getL2SetID(address);
    	unsigned int L2tag = getL2Tag(address);

    	for (int i = 0; i < 2; i++)
    	{
        	if (L1_cache[L1setID][i].tag == L1tag)
        	{
            		L1_cache[L1setID][i].timeStamp = cycles;
            		return *((unsigned int *)(L1_cache[L1setID][i].data + ((address & 0x0F) >> 2) * 4));
        	}
    	}

    	for (int i = 0; i < 4; i++)
    	{
        	if (L2_cache[L2setID][i].tag == L2tag)
        	{
            		L2_cache[L2setID][i].timeStamp = cycles;

            		if (L1lookup(address)) {
                		for (int j = 0; j < 2; j++) {
                    			if (L1_cache[L1setID][j].tag == L1tag) {
                        		L1_cache[L1setID][j].data[(address & 0x0F) >> 2] = L2_cache[L2setID][i].data[(address & 0x0F) >> 2];
                        		break;
                    			}
                		}
            		}

            		return *((unsigned int *)(L2_cache[L2setID][i].data + ((address & 0x0F) >> 2) * 4));
        	}
    	}

    	cacheBlock blockFromDRAM; // Create a cache block.

    	blockFromDRAM.tag = L2tag;
    	blockFromDRAM.timeStamp = cycles;

    	L2replace(blockFromDRAM, L2setID);
    	L1replace(blockFromDRAM, L1setID);
